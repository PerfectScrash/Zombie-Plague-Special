...
new const ambience_morpheus_sound[][] = { "zombie_plague/ambience.wav" } 
new const ambience_morpheus_dur[][] = { "17" }
...
#define DISC_CLASSNAME "destrucion_disc"

public plugin_init()
{
	// Plugin registeration.
	register_plugin("[ZP] Class Morpheus","1.0", "[P]erfec[T] [S]cr[@]s[H]")
...
public plugin_precache()
{
	...
	{
		for (i = 0; i < sizeof g_sound_morpheus; i++)
	...
	{
		for (i = 0; i < sizeof g_sound_amb_morpheus_dur; i++)
...
{
	if(is_user_alive(attacker) && zp_get_human_special_class(attacker) == g_speciald)
