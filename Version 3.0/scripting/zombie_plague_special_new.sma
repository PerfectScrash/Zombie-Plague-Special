...
public plugin_init()
{
	...
	register_cvar("zombie_plague_special", "3.0", FCVAR_SERVER|FCVAR_SPONLY)
	set_cvar_string("zombie_plague_special", "3.0")
...
public logevent_round_end()
{
	...
	if(get_pcvar_num(cvar_chosse_instantanly)) {
		for(new i = 1; i <= g_maxplayers; i++) 
			g_zombieclassnext[i] = ZCLASS_NONE
	}
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	if(!selfkill)
	{
		...
		else if(g_zombie[attacker] && g_zm_special[attacker] > 0 && g_zm_special[attacker] >= MAX_SPECIALS_ZOMBIES) {
		...
		else if(!g_zombie[attacker] && g_hm_special[attacker] > 0 && g_hm_special[attacker] >= MAX_SPECIALS_HUMANS) {
...
public fw_TakeDamage_Post(victim)
{
	...
	if(g_zombie[victim])
	{
		...
		else if(g_zm_special[victim] < MAX_SPECIALS_ZOMBIES) {
...
public fw_TraceAttack(victim, attacker, Float:damage, Float:direction[3], tracehandle, damage_type)
{
	...
	if(g_zm_special[victim] > 0 && g_zm_special[victim] < MAX_SPECIALS_ZOMBIES) {
		...
		if(Float:ArrayGetCell(g_zm_special_knockback, g_zm_special[victim]-MAX_SPECIALS_ZOMBIES) == 0.0) 
			return HAM_IGNORED;
	}
...
public menu_zclass(id, menuid, item)
{
	...
	if(g_zombie[id] && g_zm_special[id] <= 0 && g_zombieclassnext[id] == ZCLASS_NONE && get_pcvar_num(cvar_chosse_instantanly)) 
	{
...
zombieme(id, infector, classid, silentmode, rewards)
{
	...
	else if(silentmode == 2)
		g_zombieclass[id] = g_zombieclassnext[id] // For dont return to zombie classic
...
save_customization()
{
	...
	for (i = 0; i < ArraySize(g_zclass_name); i++)
	{
		...
		{
			// Get real name
			ArrayGetString(g_zclass_name, i, section, charsmax(section))
			
			if(!amx_load_setting_string(ZP_ZOMBIECLASSES_FILE, section, "NAME", section, charsmax(section)))
				amx_save_setting_string(ZP_ZOMBIECLASSES_FILE, section, "NAME", section) // Add Name
...
public ShowHUD(taskid)
{
	...
	{		
		...
		else if(g_hud_type[ID_SHOWHUD] == 3)
		{
			...
			ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync[1], "[%s]^n[%L] [%s] - [%L] [%s]^n[%L] [%s] - [%L] [%d]^n[%L] [%s]",player_name, id, "ZOMBIE_ATTRIB1", add_point(get_user_health(ID_SHOWHUD)), id, "ZOMBIE_ATTRIB5", add_point(get_user_armor(ID_SHOWHUD)), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", 
