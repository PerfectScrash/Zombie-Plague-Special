...
public show_menu_make_special(id, zombie) {
	...
	static menuid, menu[128], i, szTempid[10], buffer[32], sp_id // Create vars necessary for displaying the game modes menu
	...
	if(zombie) {
		...
		for(i = MAX_SPECIALS_ZOMBIES; i < g_zm_specials_i; i++) {
			...
			sp_id = i-MAX_SPECIALS_ZOMBIES

			if(ArrayGetCell(g_zm_special_enable, sp_id) == 0) continue;
				
			ExecuteForward(g_forwards[ZM_SP_CHOSSED_PRE], g_fwDummyResult, id, i);
			if(g_fwDummyResult >= ZP_PLUGIN_SUPERCEDE) continue;

			if(ArrayGetCell(g_zm_special_name_by_lang, sp_id)) {
				ArrayGetString(g_zm_special_lang_key, sp_id, buffer, charsmax(buffer))
				formatex(buffer, charsmax(buffer), "%L", id, buffer)
			}
			else {
				ArrayGetString(g_zm_special_name, sp_id, buffer, charsmax(buffer))
			}

			// Check for access flags and other conditions
			if(get_user_flags(id) & ArrayGetCell(g_zm_special_flags, sp_id) && g_fwDummyResult < ZP_PLUGIN_HANDLED) {
		...
		for(i = MAX_SPECIALS_HUMANS; i < g_hm_specials_i; i++) {
			sp_id = i-MAX_SPECIALS_HUMANS

			if(ArrayGetCell(g_hm_special_enable, sp_id) == 0) continue;
			...
			if(ArrayGetCell(g_hm_special_name_by_lang, sp_id)) {
				ArrayGetString(g_hm_special_lang_key, sp_id, buffer, charsmax(buffer))
				formatex(buffer, charsmax(buffer), "%L", id, buffer)
			}
			else {
				ArrayGetString(g_hm_special_name, sp_id, buffer, charsmax(buffer))
			}

			// Check for access flags and other conditions
			if(get_user_flags(id) & ArrayGetCell(g_hm_special_flags, sp_id) && g_fwDummyResult < ZP_PLUGIN_HANDLED) {
				formatex(menu, charsmax(menu), "%L", id, "MENU_MAKE_CUSTOM_SP", buffer)
				num_to_str(i, szTempid, 9);
				menu_additem(menuid, menu, szTempid)
			}
