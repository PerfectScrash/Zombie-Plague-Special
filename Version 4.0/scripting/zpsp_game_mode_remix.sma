...
// Variables
new g_gameid, g_maxplayers, cvar_minplayers, cvar_ratio, cvar_humanhp, cvar_zombiehp, g_msg_sync
...
public plugin_init()
{
	...
	register_plugin("[ZP] Remix Mode","1.0", "@bdul! | [P]erfec[T] [S]cr[@]s[H]")
...
public plugin_precache()
{
	...
	{
		for (i = 0; i < sizeof g_sound_remix; i++)
	...
	{
		for (i = 0; i < sizeof g_sound_amb_remix_duration; i++)
...
public zp_player_spawn_post(id)
{
	...
	if(zp_get_current_mode() == g_gameid)
	{
		...
		if(zp_get_user_zombie(id))
		{
			...
			zp_make_user_special(id, random_num(NEMESIS, DRAGON + zp_get_custom_special_count(GET_ZOMBIE)), GET_ZOMBIE)
		...
		{
			// Turn into a randomly special
			zp_make_user_special(id, random_num(SURVIVOR, SPY + zp_get_custom_special_count(GET_HUMAN)), GET_HUMAN)
