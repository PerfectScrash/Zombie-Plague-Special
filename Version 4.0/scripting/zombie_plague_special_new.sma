...
Array:g_hm_special_aurarad, Array:g_hm_special_glow, Array:g_hm_special_r, Array:g_hm_special_g, Array:g_hm_special_b, Array:g_hm_special_enable
...
Array:g_zm_special_allow_burn, Array:g_zm_special_allow_frost, Array:g_zm_special_enable
...
new g_menu_data[33][12], g_ent_playermodel[33], g_ent_weaponmodel[33], g_burning_dur[33], Float:g_current_maxspeed[33], g_user_custom_speed[33]
...
model_vknife_admin_human[64], model_pknife_admin_human[64], model_vknife_vip_human[64], model_pknife_vip_human[64], model_vknife_admin_zombie[64], model_vknife_vip_zombie[64], sprite_grenade_trail[64], sprite_grenade_ring[64], sprite_grenade_fire[64],
...
cvar_flaregrenades, cvar_knockbackducking, cvar_knockbackdamage, cvar_knockbackzvel, cvar_multiratio, cvar_flaresize[2], cvar_spawndelay, cvar_extraantidote, cvar_extramadness, cvar_extraantidote_ze, cvar_extramadness_ze,
...
cvar_flashshowall, cvar_hudicons, cvar_startammopacks, cvar_random_primary, cvar_random_secondary, cvar_buyzonetime, cvar_antidotelimit, cvar_madnesslimit, 
...
cvar_zm_allow_frost[MAX_SPECIALS_ZOMBIES], cvar_zm_allow_burn[MAX_SPECIALS_ZOMBIES], cvar_block_zm_use_button, cvar_zombieescapefail
...
public plugin_precache()
{
	...
	register_plugin("Zombie Plague Special", "4.0", "MeRcyLeZZ | @bdul! | [P]erfect [S]crash")
...
public plugin_init()
{
	...
	register_cvar("zombie_plague_special", "4.0", FCVAR_SERVER|FCVAR_SPONLY)
	set_cvar_string("zombie_plague_special", "4.0")
	...
	g_forwards[MODEL_CHANGE_PRE] = CreateMultiForward("zp_user_model_change_pre", ET_IGNORE, FP_CELL, FP_STRING)
	g_forwards[MODEL_CHANGE_POST] = CreateMultiForward("zp_user_model_change_post", ET_CONTINUE, FP_CELL, FP_STRING)
...
public fw_PlayerKilled_Post(victim, attacker, shouldgib)
{
	...
	if(get_pcvar_num(cvar_deathmatch) || ((g_currentmode > MODE_LNJ) && (g_deathmatchmode > 0)))
	{
		...
		if(!get_pcvar_num(cvar_respawnafterlast) && fnGetHumans() <= 1 && g_currentmode == MODE_INFECTION
		|| !get_pcvar_num(cvar_respawnafterlast) && fnGetHumans() <= 1 && g_currentmode == MODE_MULTI)
			return;
...
public show_menu_buy1(taskid)
{
	...
	if(get_pcvar_num(cvar_random_primary) || g_isbot[id])
	{
		buy_primary_weapon(id, random_num(0, ArraySize(g_primary_items) - 1))
		show_menu_buy2(taskid)
		return;
	}
...
show_menu_buy2(taskid)
{
	...
	if(get_pcvar_num(cvar_random_secondary) || g_isbot[id])
	{
...
public show_menu_admin(id)
{
	...
	if(userflags & (g_access_flag[ACCESS_MODE_NEMESIS] | g_access_flag[ACCESS_MAKE_NEMESIS]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_SURVIVOR] | g_access_flag[ACCESS_MAKE_SURVIVOR]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_SNIPER] | g_access_flag[ACCESS_MAKE_SNIPER]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_ASSASSIN] | g_access_flag[ACCESS_MAKE_ASSASSIN]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_BERSERKER] | g_access_flag[ACCESS_MAKE_BERSERKER]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_PREDATOR] | g_access_flag[ACCESS_MAKE_PREDATOR]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_WESKER] | g_access_flag[ACCESS_MAKE_WESKER]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_BOMBARDIER] | g_access_flag[ACCESS_MAKE_BOMBARDIER]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_SPY] | g_access_flag[ACCESS_MAKE_SPY]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(userflags & (g_access_flag[ACCESS_MODE_DRAGON] | g_access_flag[ACCESS_MAKE_DRAGON]) && g_fwDummyResult < ZP_PLUGIN_HANDLED)
...
show_menu2_admin(id)
{
	...
	ExecuteForward(g_forwards[GM_SELECTED_PRE], g_fwDummyResult, id, MODE_MULTI);
	if(g_fwDummyResult >= ZP_PLUGIN_SUPERCEDE) {
		if((userflags & g_access_flag[ACCESS_MODE_MULTI]) && allowed_multi() && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(g_fwDummyResult >= ZP_PLUGIN_SUPERCEDE) {
		ExecuteForward(g_forwards[GM_SELECTED_PRE], g_fwDummyResult, id, MODE_SWARM);
		if((userflags & g_access_flag[ACCESS_MODE_SWARM]) && allowed_swarm() && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	if(zm_special_enable[NEMESIS] && hm_special_enable[SURVIVOR]) {
		...
		if(g_fwDummyResult >= ZP_PLUGIN_SUPERCEDE) {
			ExecuteForward(g_forwards[GM_SELECTED_PRE], g_fwDummyResult, id, MODE_PLAGUE);
			if((userflags & g_access_flag[ACCESS_MODE_PLAGUE]) && allowed_plague() && g_fwDummyResult < ZP_PLUGIN_HANDLED)
		...
		if(g_fwDummyResult >= ZP_PLUGIN_SUPERCEDE) {
			ExecuteForward(g_forwards[GM_SELECTED_PRE], g_fwDummyResult, id, MODE_LNJ);
			if((userflags & g_access_flag[ACCESS_MODE_LNJ]) && allowed_lnj() && g_fwDummyResult < ZP_PLUGIN_HANDLED)
	...
	switch (key)
	{
		case 0: // Multiple Infection command
		{
			...
			if(userflags & g_access_flag[ACCESS_MODE_MULTI]) {
				if(allowed_multi() || g_fwDummyResult < ZP_PLUGIN_HANDLED) command_multi(id)
				else zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT")
			}
			else zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT_ACCESS")
		...
		case 1: // Swarm Mode command
		{
			...
			if(userflags & g_access_flag[ACCESS_MODE_SWARM]) {
				if(allowed_swarm() || g_fwDummyResult < ZP_PLUGIN_HANDLED) command_swarm(id)
				else zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT")
			}
			else zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT_ACCESS")
		...
		case 2: // Plague Mode command
		{
			if(zm_special_enable[NEMESIS] && hm_special_enable[SURVIVOR]) {
				...
				if(userflags & g_access_flag[ACCESS_MODE_PLAGUE]) {
					if(allowed_plague() || g_fwDummyResult < ZP_PLUGIN_HANDLED) command_plague(id)
					else zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT")
				}
				else zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT_ACCESS")
			}
		...
		case 3: // Armageddon Mode command
		{
			if(zm_special_enable[NEMESIS] && hm_special_enable[SURVIVOR]) {
				...
				if(userflags & g_access_flag[ACCESS_MODE_LNJ]) {
					if(allowed_lnj() || g_fwDummyResult < ZP_PLUGIN_HANDLED) command_lnj(id)
					else zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT")
				}
				else zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT_ACCESS")
			}			
...
start_sniper_mode(id, mode)
{
	...
	if(g_escape_map && mode == MODE_NONE) {
...
{
	if(!hm_special_enable[SURVIVOR] || g_escape_map && mode == MODE_NONE) {
		start_assassin_mode(0, MODE_NONE)
		return;
	}
...
{
	if(!hm_special_enable[BERSERKER] || g_escape_map && mode == MODE_NONE) {
...
{
	if(!hm_special_enable[WESKER] || g_escape_map && mode == MODE_NONE) {
...
{
	if(!hm_special_enable[SPY] || g_escape_map && mode == MODE_NONE) {
...
load_customization_from_files()
{
	...
	if(g_same_models_for_all) 
	{
		...
		for (i = 0; i < ArraySize(g_zclass_playermodel); i++) {
			...
			ArrayPushCell(g_zclass_modelindex, precache_player_model(key))			
		}
...
allowed_plague()
{
	if(!hm_special_enable[SURVIVOR] || !zm_special_enable[NEMESIS] || g_endround || !g_newround || task_exists(TASK_WELCOMEMSG) || floatround((fnGetAlive()-(get_pcvar_num(cvar_plaguenemnum)+get_pcvar_num(cvar_plaguesurvnum)))*get_pcvar_float(cvar_plagueratio), floatround_ceil) < 1
...
allowed_lnj()
{
	if(!hm_special_enable[SURVIVOR] || !zm_special_enable[NEMESIS] || g_endround || !g_newround || task_exists(TASK_WELCOMEMSG) || fnGetAlive() < 2)
		return false;
...
command_survivor(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_SURVIVOR, id)
	}
	else
	{
		// Turn player into a Survivor
	...
	ExecuteForward(g_forwards[HM_SP_CHOSSED_POST], g_fwDummyResult, id, SURVIVOR);
}
...
command_nemesis(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_NEMESIS, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[ZM_SP_CHOSSED_POST], g_fwDummyResult, id, NEMESIS);
}
...
command_sniper(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_SNIPER, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[HM_SP_CHOSSED_POST], g_fwDummyResult, id, SNIPER);
}
...
command_assassin(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_ASSASSIN, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[ZM_SP_CHOSSED_POST], g_fwDummyResult, id, ASSASSIN);
}
...
command_predator(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_PREDATOR, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[ZM_SP_CHOSSED_POST], g_fwDummyResult, id, PREDATOR);
}
...
command_bombardier(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_BOMBARDIER, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[ZM_SP_CHOSSED_POST], g_fwDummyResult, id, BOMBARDIER);
}
...
command_dragon(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_DRAGON, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[ZM_SP_CHOSSED_POST], g_fwDummyResult, id, DRAGON);
}
...
command_berserker(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_BERSERKER, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[HM_SP_CHOSSED_POST], g_fwDummyResult, id, BERSERKER);
}
...
command_wesker(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_WESKER, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[HM_SP_CHOSSED_POST], g_fwDummyResult, id, WESKER);
}
...
command_spy(id, player)
{
	...
	if(g_newround)
	{
		...
		ExecuteForward(g_forwards[GAME_MODE_SELECTED], g_fwDummyResult, MODE_SPY, id)
	}
	else
	{
	...
	ExecuteForward(g_forwards[HM_SP_CHOSSED_POST], g_fwDummyResult, id, SPY);
}
...
public native_get_gamemode_id(const name[])
{
	...
	new gameid, custom_gamemode_name[32]
	for (i = MAX_GAME_MODES; i < g_gamemodes_i; i++)
	{
		gameid = i-MAX_GAME_MODES
...
public native_register_human_special(const name[], const model[], hp, speed, Float:gravity, flags, clip_type, aura_radius, glow, r, g, b)
{
	...
	if(g_hm_specials_i > MAX_SPECIALS_HUMANS) {
		static szRealname2[32], alterady_exists
		alterady_exists = false
		for (new i = MAX_SPECIALS_HUMANS; i < g_hm_specials_i; i++)
		{
			if(ArrayGetCell(g_hm_specials, i-MAX_SPECIALS_HUMANS))
			{
				// Add real name
				ArrayGetString(g_hm_special_realname, i-MAX_SPECIALS_HUMANS, szRealname2, charsmax(szRealname2))
				if(equal(szRealname, szRealname2)) alterady_exists = true
			}
		}
		if(alterady_exists) {
			log_error(AMX_ERR_PARAMS, "Custom Human Special Class Alterady Exists")
			return -1;
		}
	}
	...
	new szName[32], szModel[64], value, Float:value_f
	if(amx_load_setting_int(ZP_SPECIAL_CLASSES_FILE, section, "ENABLE", value)) {
	...
	if(ArrayGetCell(g_hm_special_enable, specialid) >= 1) {
		...
		ArrayPushCell(g_hm_special_modelindex, precache_player_model(szModel))
	}
...
public native_register_zombie_special(const name[], const model[], const knifemodel[], const pain_sound[], hp, speed, Float:gravity, flags, Float:knockback, aura_radius, glow, r, g, b)
{
	...
	if(ArrayGetCell(g_zm_special_enable, specialid) >= 1) {
		...
		ArrayPushCell(g_zm_special_modelindex, precache_player_model(szModel))
	}
...
// Native: zp_zombie_class_textadd / zp_extra_item_textadd
...
public native_register_zombie_class(const name[], const info[], const model[], const clawmodel[], hp, speed, Float:gravity, Float:knockback)
{
	...
	for (i = 0; i < size; i++)
	{
		...
		if(!g_same_models_for_all)
		{
			...
			for (k = 0; k < min(nummodels_custom, nummodels_default); k++)
			{
				...
				ArraySetCell(g_zclass_modelindex, ArrayGetCell(g_zclass_modelsstart, g_zclass_i) + k, precache_player_model(buffer))
			}
			...
			if(nummodels_custom > nummodels_default)
			{
				for (k = nummodels_default; k < nummodels_custom; k++)
				{
					...
					ArrayInsertCellAfter(g_zclass_modelindex, ArrayGetCell(g_zclass_modelsstart, g_zclass_i) + k - 1, precache_player_model(buffer))
				}
	...
	if(ArrayGetCell(g_zclass_new, g_zclass_i))
	{
		...
		if(!g_same_models_for_all)
		{
			...
			ArraySetCell(g_zclass_modelindex, ArrayGetCell(g_zclass_modelsstart, g_zclass_i), precache_player_model(model))
		}
...
public native_get_special_class_id(is_zombie, const name[])
{
	...
	if(is_zombie) {
		...
		for (i = MAX_SPECIALS_ZOMBIES; i < g_zm_specials_i; i++)
		{
			ArrayGetString(g_zm_specials, i, class_name, charsmax(class_name))
		...
		for (i = MAX_SPECIALS_HUMANS; i < g_hm_specials_i; i++)
		{
			ArrayGetString(g_hm_specials, i, class_name, charsmax(class_name))
...
public menu_hud_config_type(id)
{
	...
	for(new i = 0; i < 6; i++) {	
		if(g_hud_type[id] == i) 
			formatex(szText, charsmax(szText), "\d%L [\rX\d]", id, hud_langs[i] )
		else 
			formatex(szText, charsmax(szText), "\w%L \d[]", id, hud_langs[i])
...
public menu_flashlight_color(id)
{
	...
	formatex(szText, charsmax(szText), "%L", id, "MENU_COLOR_DEFAULT")
	menu_additem(menu, szText, "0", 0)
	
	for(new i = 1; i < 8; i++) {
		formatex(szText, charsmax(szText), "%L", id, personal_color_langs[i-1])
		num_to_str(i, szItem, charsmax(szItem))
		menu_additem(menu, szText, szItem, 0)
	}
...
{
	if(item == MENU_EXIT || !get_pcvar_num(cvar_flashlight_menu)) {
		menu_destroy(menu)
		show_menu_personal(id)
		return PLUGIN_HANDLED
	}
...
public menu_nightvision(id)
{
	...
	if(get_pcvar_num(cvar_nvision_menu[0])) menu_additem(menu, szText, "2", 0)
...
{
	if(!get_pcvar_num(cvar_customnvg) || !get_pcvar_num(cvar_nvision_menu[zm ? 1 : 0]))
		return
...
{
	if(item == MENU_EXIT || !get_pcvar_num(cvar_nvision_menu[0])) {
		menu_destroy(menu)
		menu_nightvision(id)
		return PLUGIN_HANDLED
	}
...
{
	if(item == MENU_EXIT || !get_pcvar_num(cvar_nvision_menu[1])) {
		menu_destroy(menu)
		menu_nightvision(id)
		return PLUGIN_HANDLED
	}
...
public reset_player_models(id) 
{
	...
	if(g_handle_models_on_separate_ent)
	{
		if(g_zombie[id]) {
			// Set the right model
			if(g_zm_special[id] >= MAX_SPECIALS_ZOMBIES) {
				ArrayGetString(g_zm_special_model, g_zm_special[id]-MAX_SPECIALS_ZOMBIES, newmodel, charsmax(newmodel))
				if(g_set_modelindex_offset) model_index = ArrayGetCell(g_zm_special_modelindex, g_zm_special[id]-MAX_SPECIALS_ZOMBIES)
			}
			else if(g_zm_special[id] > 0 && g_zm_special[id] < MAX_SPECIALS_ZOMBIES) {
			...
			// Set the right model
			if(g_hm_special[id] >= MAX_SPECIALS_HUMANS) {
				ArrayGetString(g_hm_special_model, g_hm_special[id]-MAX_SPECIALS_HUMANS, newmodel, charsmax(newmodel))
				if(g_set_modelindex_offset) model_index = ArrayGetCell(g_hm_special_modelindex, g_hm_special[id]-MAX_SPECIALS_HUMANS)
			}
