...
public plugin_precache()
{
	...
	native_register_extra_item2("NightVision", g_extra_costs2[EXTRA_NVISION], ZP_TEAM_HUMAN|ZP_TEAM_ZOMBIE)
...
stock fnGetRandomAlive(n, const team = 0)
{
	static iAlive, id
	iAlive = 0
...
public native_register_weapon(const name[], secondary)
{
	...
	replace_all(szRealname, charsmax(szRealname), "\w", "")
	replace_all(szRealname, charsmax(szRealname), "\d", "")
	ArrayPushString(g_weapon_realname[secondary], szRealname)
...
public native_get_random_player(team) {
	static iPlayersnum
	iPlayersnum = fnGetAlive(team)

	return fnGetRandomAlive(random_num(1, iPlayersnum), team)
}
