...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if(!is_user_valid_connected(attacker) && pev_valid(attacker)) {
		...
		if(equal(classname, "trigger_hurt") && damage >= get_user_health(victim) && (g_endround || g_newround || g_currentmode == MODE_NONE))
...
public native_register_zombie_class(const name[], const info[], const model[], const clawmodel[], hp, speed, Float:gravity, Float:knockback)
{
	...
	for (index = 0; index < g_zclass_i; index++)
	{
		...
		if (equali(szRealname, zombieclass_name))
		{
			log_error(AMX_ERR_NATIVE, "[ZP] Zombie class already registered (%s)", szRealname)
			return -1;
		}
	}
